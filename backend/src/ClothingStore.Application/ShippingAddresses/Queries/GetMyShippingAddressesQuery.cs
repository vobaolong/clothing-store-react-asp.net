...
public record GetMyShippingAddressesQuery(Guid UserId) : IRequest<IReadOnlyList<ShippingAddressDto>>;
...
{
    public async Task<IReadOnlyList<ShippingAddressDto>> Handle(GetMyShippingAddressesQuery request, CancellationToken ct)
    {
        var entities = await context.ShippingAddresses.AsNoTracking()
            .Where(x => x.UserId == request.UserId)
            .OrderByDescending(x => x.IsDefault)
            .ThenByDescending(x => x.CreatedAt)
            .ToListAsync(ct);

        return entities.Select(x => new ShippingAddressDto(
            x.Id,
            x.FullName,
            x.Phone,
            x.Province,
