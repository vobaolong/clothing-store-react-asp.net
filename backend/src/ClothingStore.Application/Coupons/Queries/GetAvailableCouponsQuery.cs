...
{
    public async Task<IReadOnlyList<CouponDto>> Handle(GetAvailableCouponsQuery request, CancellationToken ct)
    {
        var now = DateTime.UtcNow;
        return await context.Coupons.AsNoTracking()
            .Where(c => c.Status == CouponStatus.Active && c.ExpiresAt > now && (!c.StartsAt.HasValue || c.StartsAt.Value <= now))
