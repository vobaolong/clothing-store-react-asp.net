...
public record ProcessVnPayIpnCommand(VnpayPaymentResult Result, string ResponseCode, long VnpAmount) : IRequest<VnPayIpnResponseDto>;
...
{
	public async Task<VnPayIpnResponseDto> Handle(ProcessVnPayIpnCommand request, CancellationToken ct)
	{
		...
		var expectedAmount = Convert.ToInt64(Math.Round(order.TotalAmount * 100m, MidpointRounding.AwayFromZero));
