		...
		: IRequestHandler<GetMyOrderDetailQuery, OrderDetailDto>
{
	public async Task<OrderDetailDto> Handle(GetMyOrderDetailQuery request, CancellationToken ct)
	{
		...
		var deliveredAt = order.StatusHistories
				.Where(h => h.Status == OrderStatus.Delivered)
				.OrderByDescending(h => h.ChangedAt)
				.Select(h => (DateTime?)h.ChangedAt)
				.FirstOrDefault();

		var inReviewWindow = deliveredAt.HasValue && deliveredAt.Value >= DateTime.UtcNow.AddDays(-10);

		var reviewedOrderItemIds = await context.Reviews.AsNoTracking()
				.Where(r => r.UserId == request.UserId && r.OrderItem != null && r.OrderItem.OrderId == order.Id)
		...
		var items = order.Items.Select(item =>
		{
			...
			var canReview = order.Status == OrderStatus.Delivered && inReviewWindow && !hasReviewed;
