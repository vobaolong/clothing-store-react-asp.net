...
) : IRequestHandler<PlaceOrderCommand, Guid>
{
	public async Task<Guid> Handle(PlaceOrderCommand request, CancellationToken cancellationToken)
	{
		if (!string.IsNullOrWhiteSpace(request.IdempotencyKey))
		{
			...
			if (memoryCache.TryGetValue($"vnpay_idempotency_{request.IdempotencyKey.Trim()}", out Guid cachedOrderId))
			{
		...
		{
			var address = await context.ShippingAddresses.FirstOrDefaultAsync(
		...
		if (request.PaymentMethod == PaymentMethod.VNPAY)
		{
			...
			if (!string.IsNullOrWhiteSpace(request.IdempotencyKey))
			{
				memoryCache.Set($"vnpay_idempotency_{request.IdempotencyKey.Trim()}", order.Id, cacheEntryOptions);
			}
		...
		catch (DbUpdateException ex) when (!string.IsNullOrWhiteSpace(request.IdempotencyKey) &&
			(ex.InnerException?.Message.Contains("IdempotencyKey") == true || ex.Message.Contains("IdempotencyKey")))
		{
			var existingOrder = await context.Orders
					.FirstOrDefaultAsync(x => x.IdempotencyKey == request.IdempotencyKey, cancellationToken);
			if (existingOrder != null)
