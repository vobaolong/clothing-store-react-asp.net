...
{
	public async Task<int> Handle(BulkUpdateOrderStatusCommand request, CancellationToken cancellationToken)
	{
		...
		var allItemVariantIds = orders.SelectMany(o => o.Items).Select(i => i.ProductVariantId).Distinct().ToList();
		var allItemProductIds = orders.SelectMany(o => o.Items).Select(i => i.ProductId).Distinct().ToList();

		var variantsMap = await context.ProductVariants
				.Where(v => allItemVariantIds.Contains(v.Id))
				.ToDictionaryAsync(v => v.Id, cancellationToken);

		var productsMap = await context.Products
				.Where(p => allItemProductIds.Contains(p.Id))
				.ToDictionaryAsync(p => p.Id, cancellationToken);

		foreach (var order in orders)
		{
			bool justDelivered = request.Status == OrderStatus.Delivered && order.Status != OrderStatus.Delivered;

			if (request.Status == OrderStatus.Delivered && order.Status != OrderStatus.Delivered)
			{
				foreach (var item in order.Items)
				{
					if (variantsMap.TryGetValue(item.ProductVariantId, out var variant)) variant.Quantity -= item.Quantity;
					if (productsMap.TryGetValue(item.ProductId, out var product)) product.SoldCount += item.Quantity;
				}
