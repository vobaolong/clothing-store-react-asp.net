...
) : INotificationHandler<OrderStatusChangedDomainEvent>
{
	public async Task Handle(OrderStatusChangedDomainEvent notification, CancellationToken cancellationToken)
	{
		try
		{
			...
			var (customerTitle, customerMessage, notificationType) = GetCustomerContent(order.Id, newStatus);
	...
	private static (string title, string message, NotificationType type) GetCustomerContent(Guid orderId, OrderStatus status)
	{
