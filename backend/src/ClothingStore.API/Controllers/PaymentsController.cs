...
) : BaseApiController
{
	...
	[Authorize]
	public async Task<IActionResult> CreateVnPayUrl(CreateVnPayUrlRequest request, CancellationToken ct)
	{
	...
	public async Task<IActionResult> HandleVnPayReturn(CancellationToken ct)
	{
		...
		var result = await sender.Send(new ProcessVnPayReturnCommand(paymentResult, responseCode), ct);
	...
	public async Task<IActionResult> ProceedAfterPayment(CancellationToken ct)
	{
		...
		var result = await sender.Send(new ProcessVnPayIpnCommand(paymentResult, responseCode, vnpAmount), ct);
		return Ok(result);
	}
}
