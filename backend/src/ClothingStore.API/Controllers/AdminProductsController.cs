...
public class AdminProductsController(ISender sender) : BaseApiController
{
	...
	[HttpPost]
	public async Task<IActionResult> Create([FromBody] AdminProductUpsertDto request, CancellationToken ct)
	{
		var id = await sender.Send(new CreateProductCommand(
				...
				request.SalePrice,
				request.CategoryId,
				request.Variants.Select(v => new CreateProductVariant(
					v.Sku,
						v.Size,
						v.Color,
						v.Hex,
	...
	public async Task<IActionResult> ImportFromExcel([FromForm] IFormFile? file, CancellationToken ct)
	{
		...
		{
			rows.Add(new AdminProductImportRowDto
			{
				RowNumber = row,
				...
				Price = worksheet.Cell(row, 11).TryGetValue<decimal>(out var price) ? price : -1,
				StockQuantity = worksheet.Cell(row, 12).TryGetValue<int>(out var stock) ? stock : -1,
	...
	[HttpPut("{id:guid}")]
	public async Task<IActionResult> Update(Guid id, [FromBody] AdminProductUpsertDto request, CancellationToken ct)
	{
	...
	public async Task<IActionResult> UpdateActive(Guid id, [FromBody] JsonElement body, CancellationToken ct)
	{
		if (!body.TryGetProperty("isActive", out var isActiveProp) || (isActiveProp.ValueKind != JsonValueKind.True && isActiveProp.ValueKind != JsonValueKind.False))
	...
	[HttpPut("bulk")]
	public async Task<IActionResult> BulkSetActive([FromBody] JsonElement body, CancellationToken ct)
	{
	...
	public async Task<IActionResult> BulkRestore([FromBody] IReadOnlyList<Guid> ids, CancellationToken ct)
	{
		if (ids == null || ids.Count == 0) return BadRequest("No product ids provided.");
	...
	public async Task<IActionResult> DeleteBulkPermanent([FromBody] IReadOnlyList<Guid> ids, CancellationToken ct)
	{
		if (ids == null || ids.Count == 0) return BadRequest("No product ids provided.");
