...
[Authorize]
public class ShippingAddressesController(ISender sender, IUserContext userContext, IApplicationDbContext context) : BaseApiController
{
	[HttpGet]
	...
	[HttpPost]
	public async Task<IActionResult> Create(UpsertShippingAddressRequest request, CancellationToken ct)
	{
		var userId = userContext.GetRequiredUserId();
		var id = await sender.Send(new UpsertShippingAddressCommand(
				null,
				userId,
				request.FullName,
				request.Phone,
	...
	[HttpPut("{id:guid}")]
	public async Task<IActionResult> Update(Guid id, UpsertShippingAddressRequest request, CancellationToken ct)
	{
		var userId = userContext.GetRequiredUserId();
		await sender.Send(new UpsertShippingAddressCommand(
				id,
				userId,
				request.FullName,
				request.Phone,
