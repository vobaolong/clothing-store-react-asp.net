...
public class AuthController(ISender sender) : BaseApiController
{
	...
	[HttpPost("verify-otp")]
	public async Task<IActionResult> VerifyOtp(VerifyRegisterOtpCommand command, CancellationToken ct)
	{
		await sender.Send(command, ct);
	...
	[HttpPost("resend-otp")]
	public async Task<IActionResult> ResendOtp(ResendRegisterOtpCommand command, CancellationToken ct)
	{
		await sender.Send(command, ct);
