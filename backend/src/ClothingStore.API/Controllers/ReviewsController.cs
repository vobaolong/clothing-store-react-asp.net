...
public class ReviewsController(ISender sender, IUserContext userContext) : BaseApiController
{
	...
	public async Task<IActionResult> CreateReview(CreateReviewRequest request, CancellationToken ct)
	{
		...
		var data = await sender.Send(
						new CreateReviewCommand(userId, request.ProductId, request.OrderItemId, request.Rating, request.Comment, request.Tags),
						ct);
	...
	public async Task<IActionResult> UpdateReview(Guid id, UpdateReviewRequest request, CancellationToken ct)
	{
		var userId = userContext.GetRequiredUserId();
		var data = await sender.Send(new UpdateReviewCommand(userId, id, request.Rating, request.Comment), ct);
...
public record CreateReviewRequest(Guid ProductId, Guid OrderItemId, int Rating, string? Comment, string[]? Tags);
