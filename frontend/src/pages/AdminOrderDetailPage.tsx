...
import { CancellationRequestStatus, CouponDiscountType, OrderStatus } from '@/enums'
...
const CANCEL_STATUS_COLORS: Record<string, string> = {
	[CancellationRequestStatus.PENDING]: 'gold',
	[CancellationRequestStatus.ACCEPTED]: 'green',
	[CancellationRequestStatus.REJECTED]: 'red'
}

const CANCEL_STATUS_I18N = {
	[CancellationRequestStatus.PENDING]: 'order.cancelStatus.pending',
	[CancellationRequestStatus.ACCEPTED]: 'order.cancelStatus.accepted',
...
export default function AdminOrderDetailPage() {
	...
	const approveCancelMutation = useMutation({
		mutationFn: approveAdminCancellationRequest,
		onSuccess: async () => {
			toast.success(t('order.cancelRequestApproved'))
			await invalidateOrder()
		},
		onError: () => {
			toast.error(t('order.cancelRequestApproveFailed'))
		}
	})

	const rejectCancelMutation = useMutation({
		mutationFn: ({ requestId, reason }: { requestId: string; reason: string }) =>
			rejectAdminCancellationRequest(requestId, { rejectionReason: reason }),
		onSuccess: async () => {
			toast.success(t('order.cancelRequestRejected'))
			setRejectModalOpen(false)
			setRejectReason('')
			await invalidateOrder()
		},
		onError: () => {
			toast.error(t('order.cancelRequestRejectFailed'))
		}
	})
	...
	const cancellationRequest = detail?.cancellationRequest ?? null
	const subtotal =
	...
	return (
		<div className="space-y-4!">
							...
							<Card
								...
								title={
									<div className="flex items-center gap-2">
										<span>{t('order.cancellationRequest')}</span>
										<Tag color={CANCEL_STATUS_COLORS[cancellationRequest.status]}>
											{t(CANCEL_STATUS_I18N[cancellationRequest.status])}
										</Tag>
									</div>
								}
								extra={
									cancellationRequest.status === CancellationRequestStatus.PENDING ? (
										<div className="flex gap-2">
											<Button
												...
												onClick={() =>
													Modal.confirm({
														title: t('order.approveCancelConfirm'),
														okText: t('order.approve'),
														cancelText: t('common.cancel'),
														onOk: () =>
															approveCancelMutation.mutateAsync(
							...
							>
								<Descriptions column={1} size="small">
									...
									{cancellationRequest.reviewedAt && (
										<Descriptions.Item label={t('order.reviewedAt')}>
											{formatDate(cancellationRequest.reviewedAt)}
										</Descriptions.Item>
									)}
									{cancellationRequest.rejectionReason && (
										<Descriptions.Item label={t('order.rejectionReason')}>
			...
			<Modal
				...
				title={t('order.rejectCancelTitle')}
				okText={t('order.reject')}
				okButtonProps={{ danger: true, loading: rejectCancelMutation.isPending }}
				cancelText={t('common.cancel')}
				onOk={() => {
					if (!cancellationRequest) return
					if (!rejectReason.trim()) {
						toast.error(t('order.rejectionReasonRequired'))
						return
					}
			...
			>
				<Input.TextArea
					...
					placeholder={t('order.rejectionReasonPlaceholder')}
				/>
			</Modal>
		</div>
	)
