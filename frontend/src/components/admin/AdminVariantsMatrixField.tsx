...
import { DEFAULT_COLORS, DEFAULT_SIZES, DEFAULT_SHOE_SIZES } from '@/constants/product'
...
>(function AdminVariantsMatrixField(
	...
	const customSizes = matrixData.sizes.filter(
		(s) => {
			const availableSizes = productType === 'shoes' ? DEFAULT_SHOE_SIZES : DEFAULT_SIZES
		...
		<div className="space-y-4">
			...
			{!isAccessories && (
				<div className="p-4 border rounded-xl border-slate-200 bg-slate-50/30">
					...
					<div className="flex flex-wrap gap-2 mb-4">
						{(productType === 'shoes' ? DEFAULT_SHOE_SIZES : DEFAULT_SIZES).map((size) => (
							<CheckableTag
								key={size}
